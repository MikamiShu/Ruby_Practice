...
Capybara::SpecHelper.spec '#switch_to_window', requires: [:windows] do
  ...
  it 'should raise error when invoked with window and block' do
    expect do
      @session.switch_to_window(@window) { @session.name == 'Title of the first popup' }
  ...
  context 'with block' do
    ...
    it 'should return window' do
      window = @session.switch_to_window { @session.name == 'Title of popup two' }
    ...
    it "should raise error if window matching block wasn't found" do
      ...
      expect do
        @session.switch_to_window { @session.name == 'A name' }
  ...
  it 'should wait for window to appear' do
    ...
    expect do
      @session.switch_to_window(wait: 5) { @session.name == 'Title of the first popup' }
